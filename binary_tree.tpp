...
void BinaryTree<TreeItemType, FunctionType>::attachRight(
    ...
    if(root == nullptr){
        ...
        throw std::logic_error("Cannot overwrite subtree");
    }
    else{
...
void BinaryTree<TreeItemType, FunctionType>::copyTree(
  ...
  // preorder traversal
  // start from the root, then left, then right
  if (treePtr != nullptr) { 
    //TODO
    // copy root node
...
BinaryTree<TreeItemType, FunctionType> BinaryTree<TreeItemType, FunctionType>::makeBinaryTree(const TreeItemType *array, int low, int high) {
    ...
    //TODO
    // function takes the items between index of low and high in the array and makes a binary tree with them
    if(low > high){  // The array is empty, under
        root = nullptr;
        return *this;
    } 
    else if(low == high){  // There is only one item in the array
        root = new NodeType(array[low]);
    }
    
    // Calculation of the mid value and creation of the left and right subtrees
    int mid = low + (high - low)/2;
    BinaryTree left;
    left.makeBinaryTree(array, low, mid-1);

    BinaryTree right;
    right.makeBinaryTree(array, mid+1, high);

    // Let the mid point between low and high become the new root node
    root = new NodeType(array[mid]);
    
    // update the leftchild/subtree
    if(!left.isEmpty()){
        left.attachLeftSubtree(left);
    }

    // update the rightchild/subtree
    if(!right.isEmpty()){
        right.attachRightSubtree(right);
    }
    return *this;
